...
def analyze_and_create_offer(company_info):
    ...
    prompt = (
        f"Ты — опытный SMM-маркетолог, пишущий холодные предложения в Директ. Твоя цель — зацепить бизнес. "
        f"Посмотри на эту компанию: {company_info}. "
        f"Напиши СТРОГО КОРКИЙ, ТЕЗИСНЫЙ ОФФЕР (максимум 150 слов). "
        f"Структура предложения:\n"
        f"1. Короткое приветствие и комплимент по сути (без банальщины).\n"
        f"2. 2-3 конкретные точки роста, которые ты видишь (например: исправить хайлайтс, добавить трендовые Reels для привлечения трафика, сделать единый визуал).\n"
        f"3. Четкое предложение: что конкретно ты сделаешь для них и какой результат это даст.\n"
        f"4. Призыв к действию (CTA): предложить созвониться на 10 минут или отправить бесплатный аудит.\n"
        f"Пиши максимально емко, профессионально, без лишней «воды» и пустых комплиментов. Используй списки и эмодзи."
    )
